...
public function login(){
    $dbs = new DatabaseService("compte_client");
    ...
    if(count($accounts) == 1
       && password_verify($this->body['mot_de_passe'], $prefix . $accounts[0]->password)){
       $dbs = new DatabaseService("appUser");
       $appUser = $dbs->selectWhere($accounts[0]->Id_appUser);
        
        $secretKey = $_ENV['config']->jwt->secret;
        $issuedAt = time();
        $expireAt = $issuedAt + 60 * 60 * 24;
        $serverName = "e-boutique";
